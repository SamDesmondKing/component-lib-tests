...
export function FloatingActionBar({ count, onDeactivate, onDelete }: FloatingActionBarProps) {
  return createPortal(
    <div className="floating-action-bar" role="toolbar" aria-label="Bulk actions">
      ...
      <Button className="floating-action-bar__btn floating-action-bar__btn--deactivate" onPress={onDeactivate}>
        Deactivate
      </Button>
      <Button className="floating-action-bar__btn floating-action-bar__btn--delete" onPress={onDelete}>
        Delete
      </Button>
    </div>,
    document.body
