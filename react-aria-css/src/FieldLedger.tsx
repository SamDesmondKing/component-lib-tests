	...
	return (
		<tr ref={setNodeRef} style={style} className="field-ledger__row">
			<td className="field-ledger__cell field-ledger__cell--drag">
				<button
					className="field-ledger__drag-handle"
			...
			<td className="field-ledger__cell field-ledger__cell--mono">
				{field.name}
			</td>
			<td className="field-ledger__cell">{field.type}</td>
			<td className="field-ledger__cell">
				<span
					className={`field-ledger__badge field-ledger__badge--${field.status}`}
				>
					{field.status}
				</span>
			</td>
			<td className="field-ledger__cell field-ledger__cell--num">
				{field.usageCount}
			</td>
		</tr>
	);
...
export function FieldLedger({ fields, onFieldsChange }: FieldLedgerProps) {
	...
	const handleDeactivate = () => {
		onFieldsChange(
			fields.map((f) =>
				selection.has(f.id) ? { ...f, status: "inactive" as const } : f,
			),
		);
		setSelection(new Set());
	};
							...
							<thead>
								<tr>
									...
									<th className="field-ledger__th field-ledger__th--check">
										<Checkbox
											isSelected={
												sorted.length > 0 && selection.size === sorted.length
											}
											isIndeterminate={
									...
									{columns.map((col) => (
										...
										>
											{col.label}
											{sortKey === col.key && (
												<span className="field-ledger__sort-icon">
													{sortDir === "asc" ? " ▲" : " ▼"}
												</span>
											)}
										</th>
